...
fn main() {
    ...
    //Source: Exploration: Threads code
    let xs2 = xs.clone();

    //Creating 2 new threads by calling function thread::spawn 
    // each uses map_data() function to process one of the two partitions 
    let thread1 = thread::spawn(move||map_data(&xs[0]));
    let thread2 = thread::spawn(move||map_data(&xs2[1]));

    //Joining thread by caling join on the join handle
    //We wait for tht thread to complete its execution by calling the function join
    //on the join handle, hthen call the function unwrap
    let res1 = thread1.join().unwrap();
    let res2 = thread2.join().unwrap();

    //adds elements to vector by using the push function, adds value to end of the vector
    ...
    for i in 0..num_partitions {
        ...
        //Creating new thread by calling thread::spawn (t1)
        //then waits for them to terminate by calling join on each thread's join handle 
        //then calls fhe unwrap function 
        let t1 = thread::spawn(move||map_data(&partitions_clone[i]));
        let _r1 = t1.join().unwrap();
        
        //pushing the results of map_data to intermediate_sums2 vector
        intermediate_sums2.push(_r1);
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    // len is lengh of partition 
    let partition_size = v.len() / num_partitions;

    //Initializing begin and end variables 
    let mut begin = 0;
    ...
    for i in 0..num_partitions {
        ...
        //creating a new vector of values 
        let mut x1: Vec<usize> = Vec::new();
        //Add the integers in the input vector to x1
        for j in begin..end{
            x1.push(v[j]);
        }
        // Add x1 to the vector that will be returned by this function 
        ...
        //resetting variables 
        if remainder <= 0 {
            begin = begin + partition_size;
            end = end+ partition_size;

        }
        //This means that the partitition size is not equal 
        //Increasing begin and end variables by 1 until there is not remainder 
        else{
            xs[i].push(v[end]);
            remainder = remainder - 1;
            begin = begin + partition_size + 1;
            end = end + partition_size + 1;
        }
    }
